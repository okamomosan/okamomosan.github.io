...
- しかし、２章で説明したプロジェクト機能を使っていれば、プロジェクトフォルダが作業ディレクトリになる（よって、**プロジェクトフォルダにデータファイルを置けば良い**）  
...
- １章で作った`zemi`のプロジェクトフォルダ内に`data`というフォルダを作る
- 以下から[`sokutei.csv`](https://www.dropbox.com/s/8fy2dbnurinract/shitaisokutei.csv?dl=0 )をダウンロードし、上の`data`フォルダ内に置く
- このデータを読み込み、`sokutei_csv`というオブジェクトに格納する
- `head()`を使って`sokutei_csv`の中身を確認する
- 以下から[`sokutei.xls`](https://www.dropbox.com/s/m4m4etyd81pnpq6/sokutei.xls?dl=0)をダウンロードし、上の`data`フォルダ内に置く
- このデータを読み込み、`sokutei_excel`というオブジェクトに格納する
- `head()`を使って`sokutei_excel`の中身を確認する
...
[^3]: ４章の`tidyverse`パッケージに含まれた`read_csv()`を使っても良い（この場合、データフレームによく似た`tibble`という形式で読み込まれる）