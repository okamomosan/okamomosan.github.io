...
- その中に`sokutei.csv`を置いた場合、`read_csv("data/sokutei.csv")`で読み込む

## 演習問題
  
1. １章で作った`zemi`のプロジェクトフォルダ内に`data`というフォルダを作る
1. 以下から[`sokutei.csv`](https://www.dropbox.com/s/8fy2dbnurinract/shitaisokutei.csv?dl=0 )をダウンロードし、上の`data`フォルダ内に置く
1. このデータを読み込み、`sokutei_csv`という変数に格納する
1. `head()`を使って`sokutei_csv`の中身を確認する
1. 以下から[`sokutei.xls`](https://www.dropbox.com/s/m4m4etyd81pnpq6/sokutei.xls?dl=0)をダウンロードし、上の`data`フォルダ内に置く
1. このデータを読み込み、`sokutei_excel`という変数に格納する
1. `head()`を使って`sokutei_excel`の中身を確認する
1. `sokutei_excel`から変数`weight`を抜き出し、`w`という新たな変数に格納する
1. `w`の中身を確認する
<br><br>
...
1. wooldridgeという名前のパッケージをインストールする
2. wooldridgeのパッケージを呼び出す
3. `data("wage1")`を実行して、wooldridgeに収録された`wage1`というデータを読み込む[^4]
4. `head(wage1)`を実行して、`wage1`の中身を確認する
5. `help(package="wooldridge")`を実行して、`wage1`以外にどんなデータが収録されているか確認
