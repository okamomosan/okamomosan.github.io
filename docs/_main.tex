...
\item
  しかし、２章で説明したプロジェクト機能を使っていれば、プロジェクトフォルダが作業ディレクトリになる（よって、\textbf{プロジェクトフォルダにデータファイルを置けば良い}）
\end{itemize}
...
\tightlist
\item
  csvファイルを読み込むときは、\texttt{read.csv()}を使う\footnote{４章の\texttt{tidyverse}パッケージに含まれた\texttt{read\_csv()}を使っても良い（この場合、データフレームによく似た\texttt{tibble}という形式で読み込まれる）}
\item
